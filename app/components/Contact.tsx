...
const Contact: React.FC = () => {
    ...
    const handleValidation = () => {
        ...
        const userEmailConfirm = (formElements?.namedItem('user_email_confirm') as HTMLInputElement)?.value;
    ...
    const checkFormFields = () => {
        ...
        const userEmail = (formElements?.namedItem('user_email') as HTMLInputElement)?.value;
        const userMessage = (formElements?.namedItem('user_message') as HTMLTextAreaElement)?.value;
    ...
    return (
        <section className="p-4 md:pt-32" id="contact">
            <h2 className="text-center text-4xl font-bold text-white mb-4">Contact Me</h2>
            ...
            <div className="flex flex-col justify-center items-center text-white my-2">
                    ...
                    <>
                        ...
                        <form
                            ...
                            <div className="flex justify-center">
                                <button
                                    type="submit"
                                    disabled={!captchaToken || isFormSubmitted || isSubmitting || !formFieldsFilled || !!emailError || !!emailMatchError}
                                    className="px-6 py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-md">
                                    {isSubmitting ? 'Sending...' : isFormSubmitted ? 'Message Sent' : 'Send Message'}
                                </button>
                            </div>
                        </form>
                    </>
                ...
                {status && isFormSubmitted && (
                    <div
                        role="alert"
                        aria-live="polite"
                        className={`mt-4 text-center text-xl ${status === 'Message sent successfully! ✅'
                            ? 'text-green-500'
                            : 'text-red-500'
                            }`}>
                        {status}
                ...
                {showThankYou && (
                    <div className="mt-4 text-center text-xl text-pink-500">
                        Thank you for your feedback. If a response is required, I will get back to you as soon as possible.
                    </div>
                )}
            </div>
        </section>
